...
/**
 * The program contains a series of instructions that tell the Graphic
 * Processing Unit (GPU) what to do with every vertex and fragment that we
 * transmit. The vertex shader and the fragment shaders together are called
...
/**
 * The following code snippet creates a vertex buffer and binds the vertices to
 * it.
 */
function getVertexBufferWithVertices(vertices) {
  ...
  glContext.bufferData(glContext.ARRAY_BUFFER, new Float32Array(vertices),
...
/**
 * The following code snippet creates a vertex buffer and binds the indices to
 * it.
 */
function getIndexBufferWithIndices(indices) {
  ...
  glContext.bufferData(glContext.ELEMENT_ARRAY_BUFFER, new Uint16Array(indices),
    glContext.STATIC_DRAW,
  );
...
function getArrayBufferWithArray(values) {
  //The following code snippet creates an array buffer and binds the array
  // values to it
  var vBuffer = glContext.createBuffer();
  glContext.bindBuffer(glContext.ARRAY_BUFFER, vBuffer);
  glContext.bufferData(glContext.ARRAY_BUFFER, new Float32Array(values),
    glContext.STATIC_DRAW,
  );
  glContext.bindBuffer(glContext.ARRAY_BUFFER, null);
...
function initTextureWithImage(sFilename, texturen) {
  ...
  texturen[anz].image.onload = function () {
    ...
    glContext.texImage2D(glContext.TEXTURE_2D, 0, glContext.RGBA,
      glContext.RGBA, glContext.UNSIGNED_BYTE, texturen[anz].image,
    );
  ...
  // let's use a canvas to make textures, with by default a random color (red,
  // green, blue)
  function rnd() {
  ...
  glContext.texParameteri(glContext.TEXTURE_2D, glContext.TEXTURE_WRAP_S,
    glContext.CLAMP_TO_EDGE,
  );
  glContext.texParameteri(glContext.TEXTURE_2D, glContext.TEXTURE_WRAP_T,
    glContext.CLAMP_TO_EDGE,
  );
}
